...
@implementation BXMapConvertUtil
/// 经纬度坐标转字符串
...
+ (QMapRect)mapRectFromArray:(NSArray *)array {
    NSAssert((array && [array isKindOfClass:[NSArray class]] && array.count == 2), @"解析mapRect的参数有误");
    CLLocationCoordinate2D southwest = [BXMapConvertUtil coordinateFromArray:array[0]];
    CLLocationCoordinate2D northeast = [BXMapConvertUtil coordinateFromArray:array[1]];
    QMapPoint mapNorthEastPoint = QMapPointForCoordinate(northeast);
    QMapPoint mapSouthWestPoint = QMapPointForCoordinate(southwest);
    double width  = fabs(mapNorthEastPoint.x - mapSouthWestPoint.x);
    double height = fabs(mapNorthEastPoint.y - mapSouthWestPoint.y);
    QMapRect limitRect = QMapRectMake(mapSouthWestPoint.x, mapNorthEastPoint.y, width, height);
    return limitRect;
}
@end
