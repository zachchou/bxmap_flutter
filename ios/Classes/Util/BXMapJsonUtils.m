    ...
    for (NSString *propertyName in propertyArray) {
        ...
        if(splitPropertyAttributes.count > 0) {
            ...
            if([encodeType hasPrefix:@"T@"]) {
                ...
                CGPoint point = [BXMapConvertUtil pointFromArray:value];
                //使用msgSend直接设置经纬度的属性
                SEL sel = NSSelectorFromString([NSString stringWithFormat:@"set%@:",[propertyName capitalizedString]]);
...
#pragma clang diagnostic pop
                ...
                UIColor *color = [BXMapConvertUtil colorFromNumber:value];
...
+ (CLLocationCoordinate2D)coordinateFromModel:(id)model {
    ...
    if ([model isKindOfClass:[NSArray class]]) {
        return [BXMapConvertUtil coordinateFromArray:model];
        ...
        return [BXMapConvertUtil coordinateFromArray:array];
